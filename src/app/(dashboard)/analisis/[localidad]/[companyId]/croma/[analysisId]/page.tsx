...
export default function AnalysisDetailPage({ params }: Props) {
  ...
  const { data: analysis, isLoading: loading, error: queryError } = useAnalysis(params.analysisId);
  const updateMutation = useUpdateAnalysis(params.analysisId);
  ...
  const handleToggleComponentVisibility = async (componentCode: string, visible: boolean) => {
    try {
      ...
      toast.success('Visibilidad de componente actualizada');
    } catch (error: any) {
      console.error('❌ Error guardando visibilidad:', error);
      toast.error(`Error al guardar visibilidad: ${error?.message || 'Error desconocido'}`);
    }
  };
    ...
    ? queryError instanceof Error
      ? queryError.message?.includes('401')
        ...
        : queryError.message?.includes('404')
          ? `No se encontró el análisis con ID: ${params.analysisId}`
    ...
    return (
      <div className="flex min-h-screen items-center justify-center">
        <div className="text-center">
          ...
          <button
            onClick={() => router.push(`/analisis/${params.localidad}/${params.companyId}/croma`)}
        ...
        <div className="mb-6">
          <div className="mb-4 flex items-center justify-between">
            ...
            {analysis.chroma_report_html && (
              <button
                onClick={() => router.push(`/cromatografia/${params.analysisId}/informe`)}
