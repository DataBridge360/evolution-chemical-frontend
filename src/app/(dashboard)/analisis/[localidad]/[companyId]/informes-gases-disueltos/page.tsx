...
import {
  useGasesDisueltosReports,
  useDownloadGasesDisueltosExcel,
  useUpdateGasesDisueltosReport,
} from '@/src/modules/informes/gases-disueltos/hooks';
import { ReportPreview } from '@/src/modules/informes/gases-disueltos/components/ReportPreview';
import { parseGasesDisueltosReportForPreview } from '@/src/modules/informes/gases-disueltos/utils';
import type {
  GasesDisueltosReportRecord,
  GasSamplingPoint,
} from '@/src/modules/informes/gases-disueltos/types';
...
function parseSamplingPoints(raw: string | GasSamplingPoint[] | undefined): GasSamplingPoint[] {
  if (!raw) return [];
...
function ReportDrawer({
  ...
  report: GasesDisueltosReportRecord;
  onClose: () => void;
  onReportUpdated: (updated: GasesDisueltosReportRecord) => void;
}) {
  const overlayRef = useRef<HTMLDivElement>(null);
  const excelMutation = useDownloadGasesDisueltosExcel();
  const updateMutation = useUpdateGasesDisueltosReport();
  const [isEditing, setIsEditing] = useState(false);
  const [localReport, setLocalReport] = useState(report);

  const previewData = useMemo(
    () => parseGasesDisueltosReportForPreview(localReport),
    [localReport],
  );
  ...
  const handlePointChange = (index: number, field: keyof GasSamplingPoint, value: string) => {
    const points = parseSamplingPoints(localReport.sampling_points);
    if (!points[index]) return;

    points[index] = { ...points[index], [field]: value };
    const serialized = JSON.stringify(points);

    setLocalReport((prev) => ({ ...prev, sampling_points: serialized }));

    updateMutation.mutate(
      { reportId: report.id, data: { sampling_points: serialized } },
      {
  ...
  const handleMetaChange = (fieldName: string, newValue: string) => {
    setLocalReport((prev) => ({ ...prev, [fieldName]: newValue }) as GasesDisueltosReportRecord);
  ...
  const onDownloadExcel = () => {
    ...
    excelMutation.mutate(
      {
        reportId: report.id,
        filename: `informe_gases_disueltos_${oilfield}_${reportNum}.xlsx`,
      },
      {
...
<head>
<meta charset="utf-8">
<title>Informe_GasesDisueltos_${reportNum}</title>
<style>
...
export default function InformesGasesDisueltosPage() {
  ...
  const [selectedReport, setSelectedReport] = useState<GasesDisueltosReportRecord | null>(null);

  const { data: reports = [], isLoading: loadingReports } = useGasesDisueltosReports(companyId);
      ...
      {reports.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="mt-1 text-xs text-[#a3a3a3]">
            No hay informes de gases disueltos para esta empresa.
          </p>
        </div>
      ) : (
        <div className="overflow-hidden rounded-xl border border-[#e5e5e5] bg-white">
          <table className="w-full">
            ...
            <tbody>
              {reports.map((report) => {
                const points = parseSamplingPoints(report.sampling_points);
                return (
                  <tr
                    key={report.id}
                  ...
                  >
                    ...
                    <td className="px-5 py-3.5 text-xs">{points.length}</td>
