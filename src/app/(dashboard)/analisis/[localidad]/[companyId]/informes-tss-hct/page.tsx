...
import {
  ...
  useUpdateTssHctReport,
} from '@/src/modules/informes/tss-hct/hooks';
import { ReportPreview } from '@/src/modules/informes/tss-hct/components/ReportPreview';
import { parseTssHctReportForPreview } from '@/src/modules/informes/tss-hct/utils';
import type { TssHctReportRecord, SamplingPoint } from '@/src/modules/informes/tss-hct/types';
...
function parseSamplingPoints(raw: string | SamplingPoint[] | undefined): SamplingPoint[] {
  if (!raw) return [];
...
function ReportDrawer({
  ...
  report: TssHctReportRecord;
  onClose: () => void;
  onReportUpdated: (updated: TssHctReportRecord) => void;
}) {
  const overlayRef = useRef<HTMLDivElement>(null);
  const excelMutation = useDownloadTssHctExcel();
  const updateMutation = useUpdateTssHctReport();
  const [isEditing, setIsEditing] = useState(false);
  const [localReport, setLocalReport] = useState(report);

  const previewData = useMemo(() => parseTssHctReportForPreview(localReport), [localReport]);

  const handleOverlayClick = (e: React.MouseEvent) => {
    if (e.target === overlayRef.current) onClose();
  };

  const handlePointChange = (index: number, field: keyof SamplingPoint, value: string) => {
  ...
  const handleMetaChange = (fieldName: string, newValue: string) => {
    setLocalReport((prev) => ({ ...prev, [fieldName]: newValue }) as TssHctReportRecord);
...
export default function InformesTssHctPage() {
  ...
  const [selectedReport, setSelectedReport] = useState<TssHctReportRecord | null>(null);

  const { data: reports = [], isLoading: loadingReports } = useTssHctReports(companyId);
      ...
      {reports.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="mt-1 text-xs text-[#a3a3a3]">
            No hay informes TSS + HcT para esta empresa.
          </p>
        </div>
      ) : (
