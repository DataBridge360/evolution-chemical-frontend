...
import {
  ...
  usePatchHistoric,
} from '@/src/modules/historics/fq-type-a/hooks/useHistoricFQTypeA';
import { downloadHistoricExcel } from '@/src/modules/historics/fq-type-a/services/historicFQTypeAService';
import {
  HISTORICO_COLUMN_ORDER,
  HISTORICO_COLUMN_HEADERS,
  METADATA_COLUMNS,
  FROZEN_WIDTH,
...
export default function HistoricoFQTipoAPage() {
  ...
  const totalWidth = FROZEN_WIDTH + HISTORICO_COLUMN_ORDER.length * PARAM_COL_W;
      ...
      {totalRecords === 0 ? (
        <div className="rounded-2xl border border-dashed border-[#d7e4f2] bg-[#fbfdff] px-5 py-12 text-center">
          ...
          <p className="mt-2 text-sm text-[#66788c]">
            Subi un informe desde el dashboard para agregar registros.
          </p>
        </div>
      ) : (
                  ...
                  {HISTORICO_COLUMN_ORDER.map((field, idx) => (
                    <th
                      ...
                      className={`border border-[#bcc9d8] bg-white px-2 py-2 text-center text-xs font-normal leading-snug text-[#333] ${
                        idx === FQ_PARAM_COUNT - 1 ? 'border-r-2 border-r-[#999]' : ''
                      }`}
                      style={{ minWidth: PARAM_COL_W, height: 70 }}
                    >
                      {HISTORICO_COLUMN_HEADERS[field] || field}
                    </th>
                  ))}
                </tr>
              </thead>
                    ...
                    >
                      ...
                      {HISTORICO_COLUMN_ORDER.map((field, idx) => {
                        ...
                        return (
                          <td
                            key={field}
                            className={`border border-[#d5dde7] px-2 py-3 text-center text-sm tabular-nums ${
                              idx === FQ_PARAM_COUNT - 1 ? 'border-r-2 border-r-[#999]' : ''
                            } ${display === 'NR' ? 'text-[#aaa]' : 'text-[#333]'}`}
                            style={{ minWidth: PARAM_COL_W }}
                            onDoubleClick={() =>
                              handleDoubleClick(record.id, field, rawValue as string | null)
                            }
                          >
                            {isEditing ? (
                              <input
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
          <div className="w-80 rounded-xl border border-[#d7e5f4] bg-white p-5 shadow-lg">
            ...
            <p className="mt-2 text-xs text-[#66788c]">
              {HISTORICO_COLUMN_HEADERS[pendingEdit.field] || pendingEdit.field}:{' '}
